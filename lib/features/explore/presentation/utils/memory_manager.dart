...
class InfiniteScrollMemoryManager {
  ...
  static final LRUMap<String, Recipe> _recipeCache = LRUMap<String, Recipe>(
    maxCachedRecipes,
  );
  ...
  static void initialize() {
    ...
    debugPrint(
      'MemoryManager: Initialized with monitoring every ${memoryCheckIntervalMs}ms',
    );
  }
  ...
  static void optimizeImageCache({bool aggressive = false}) {
    ...
    if (aggressive || currentSizeMB > maxMemoryThresholdMB) {
      ...
      final keepIds = sortedEntries.take(keepCount).map((e) => e.key).toSet();
      ...
      if (currentSizeMB > aggressiveCleanupThresholdMB) {
        imageCache.clear();
        debugPrint(
          'MemoryManager: Cleared entire image cache (${currentSizeMB.toStringAsFixed(1)}MB)',
        );
      }

      debugPrint(
        'MemoryManager: Image cache cleanup - removed $clearedCount entries, kept ${keepIds.length}',
      );
    }
  }
  ...
  static void cleanupOffScreenRecipes() {
    ...
    if (toRemove.isNotEmpty) {
      debugPrint(
        'MemoryManager: Cleaned up ${toRemove.length} off-screen recipes',
      );
    }
  }
  ...
  static void _performMemoryCheck() {
    ...
    if (deltaMB > 10) {
      ...
      debugPrint(
        'MemoryManager: Memory changed by ${deltaMB.toStringAsFixed(1)}MB (now ${currentMB.toStringAsFixed(1)}MB)',
      );
    }
    ...
    if (currentMB > maxMemoryThresholdMB) {
      debugPrint(
        'MemoryManager: Memory threshold exceeded (${currentMB.toStringAsFixed(1)}MB), triggering cleanup',
      );
      optimizeImageCache();
    ...
    if (currentMB > aggressiveCleanupThresholdMB) {
      debugPrint(
        'MemoryManager: Aggressive cleanup threshold reached, forcing GC',
      );
  ...
  static Map<String, dynamic> getMemoryStats() {
    ...
    return {
      ...
      'maxImageCacheSizeMB': (imageCache.maximumSizeBytes / (1024 * 1024))
          .toStringAsFixed(1),
    };
  }
