...
class UserRecipeDetailScreen extends ConsumerStatefulWidget {
  final String recipeId;

  const UserRecipeDetailScreen({super.key, required this.recipeId});

  @override
  ConsumerState<UserRecipeDetailScreen> createState() =>
      _UserRecipeDetailScreenState();
}

class _UserRecipeDetailScreenState
    extends ConsumerState<UserRecipeDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Initialize serving size from recipe
    if (currentServings == 4 && recipe.defaultServings > 0) {
      currentServings = recipe.defaultServings;
    }

    return RefreshIndicator(
      onRefresh: () async {
        ref.invalidate(userRecipeProvider(widget.recipeId));
      },
      child: _buildRecipeContent(recipe),
    );
  }
  ...
  Widget _buildRecipeContent(UserRecipe recipe) {
    return Scaffold(
      backgroundColor: AppColors.background,
  ...
  Widget _buildIngredients(UserRecipe recipe) {
    ...
    final defaultServings = recipe.defaultServings > 0
  ...
  Widget _buildInstructions(UserRecipe recipe) {
    ...
    if (recipe.instructionSections.isNotEmpty) {
      ...
      for (final section in recipe.instructionSections) {
        ...
        if (recipe.instructionSections.length > 1) {
  ...
  void _addToMealPlan(UserRecipe recipe) {
  ...
  void _addToTodaysMeals(UserRecipe recipe) {
